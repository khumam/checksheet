...
@section('content')
@include('components.breadcrumb', ['title' => 'Kelola Equipment', 'lists' => ['Home' => '/', 'Equipment' => route('admin.equipment.index'), 'Edit' => '#']])
            ...
            <div class="card-body">
                <form action="{{ route('admin.equipment.update', $equipment) }}" method="POST" enctype="multipart/form-data">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Nama Equipment" value="{{ $equipment->name }}" required>
                    @error('name')
                    ...
                    @enderror
									...
									<div id="descArea">
										...
										<div class="card" id="desc{{$desc->id}}">
											<div class="card-body">
												@if(!$loop->first)
												<div class="row">
													<div class="col text-right mb-3">
														<button class="btn btn-danger" type="button" onclick="deleteRowDescription('desc{{$desc->id}}')">Hapus</button>
													</div>
												</div>
												@endif
												<div class="row">
													<div class="col">
														<div class="form-group">
															...
															<input type="text" id="desc" name="desc[]" class="form-control @error('desc') is-invalid @enderror" value="{{ $desc->desc }}" placeholder="Deskripsi" required>
														</div>
													</div>
													<div class="col">
														<div class="form-group">
															<label for="satuan">Satuan</label>
															<input type="text" id="satuan" name="satuan[]" class="form-control @error('satuan') is-invalid @enderror" value="{{ $desc->satuan }}" placeholder="Deskripsi" required>
														</div>
													</div>
													<div class="col">
														<div class="form-group">
															<label for="standard">Standard</label>
															<input type="text" id="standard" name="standard[]" class="form-control @error('standard') is-invalid @enderror" value="{{ $desc->standard }}" placeholder="Deskripsi" required>
														</div>
													</div>
												</div>
											</div>
									...
									<div class="form-group">
										<button class="btn btn-success">Simpan Equipment</button>
									</div>
                </form>
            </div>
        </div>
